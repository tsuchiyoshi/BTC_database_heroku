...
# csv格納先フォルダを指定
folder_name = 'btc_quotes_historical_coincheck'
...
limit = 100000              #　取得単位 (max 100,000件)
max_iter = 100             # 取得回数(1日100回が上限？らしい)
...
def timeperiod_transform(timeperiod_str):
    return timeperiod_str.replace('-', '').replace(':', '').replace('T', '').replace('.', '')[:16]

# ファイルの保存用関数
def rename_and_tocsv(df):
    data_from = timeperiod_transform(df["time_exchange"][0])
    data_end = timeperiod_transform(df["time_exchange"][df.shape[0] - 1])

    new_file_name = "quotes_historical_" + data_from + "_" + data_end + ".csv"
...
if len(file_list) > 0:
    ...
    start_of = last_df.tail(1)['time_exchange'].iloc[0]
...
for i in range(max_iter):
    try:
        ...
        # データの取得
        quotes_historical = api.quotes_historical_data('COINCHECK_SPOT_BTC_JPY',
                                                      {'time_start': next_start, 'limit': limit})

        # 初回のみall_dfの作成、2周目以降はall_dfにデータ追加
        if 'all_df' in locals():
            # all_dfがある場合の処理
            df_tmp = pd.DataFrame(quotes_historical)
            # データの結合、整形
            all_df = pd.concat([all_df, df_tmp], ignore_index=True)
            all_df = all_df.drop_duplicates().reset_index(drop=True)
        else:
            # all_dfがない場合の処理
            all_df = pd.DataFrame(quotes_historical)
        ...
        # 次の取得基準時間の設定
        next_start = all_df['time_exchange'][all_df.shape[0] - 1]
        pass
